...
const CAT_PERSONALITY = {
  ...
  openaiController.createChatbotMessage = async (req, res) => {
    try {
      ...
      if (!allowedReplies || typeof allowedReplies !== "number" || allowedReplies <= 0) {
        return res.status(400).json({ error: "Invalid number of allowed replies" });
      }
  
      const { systemMessage, personalityContent } = chatbotController.chatbotMessagePersonality(catPersonality, 20);
      ...
      res.status(400).json({ error: "AI request chatbotMessage failed", rawError: error });
    }
  };
  ...
  openaiController.createPrintLine = async (req, res) => {
    try {
      ...
      const { systemMessage, personalityContent } = chatbotController.chatbotMessagePersonality(catPersonality, 10);
      ...
      res.status(400).json({ error: "AI request printLine failed", rawError: error });
    }
  };
